...
with tab1:
    ...
    with col_droite:
        ...
        # Tableau de legendes structure en colonnes web
        ...
        # Rendu dynamique du Quiz
        for q in quiz1:
            if q["type"] == "menu":
                reponses_quiz1[q["id"]] = st.selectbox(q["q"], options=q["options"], key=f"src_{q['id']}")
            else:
                reponses_quiz1[q["id"]] = st.text_input(q["q"], key=f"src_{q['id']}").strip().lower()
            
            # Gestion de l'affichage adaptatif selon le Mode Examen
            if reponses_quiz1[q["id"]] != "" and reponses_quiz1[q["id"]] != "-------":
                if reponses_quiz1[q["id"]] == q["rep"]:
                    score1 += 1
        ...
        # Bouton global de validation de l'onglet
        if st.button("Valider et enregistrer l'onglet 1"):
            if st.session_state.mode_examen_tab1:
                st.info("Reponses enregistrees pour correction. Mode examen actif.")
            else:
                st.metric("Votre score pour cet onglet", f"{score1} / {len(quiz1)}")
        # --- REPRÉSENTATION GRAPHIQUE DE L'ACIDE ACÉTIQUE (Ancien Canvas1) ---
...
with tab3:
    ...
    with col_workspace_g:
        ...
        quiz31_inputs = [
            ...
            {"id": "q31_4", "q": "Calculer la concentration molaire en vinaigre dosee en mol/L :", "rep": c_vinaigre_dose_attendu},
            {"id": "q31_5", "q": "Calculer la masse de vinaigre dosee en gramme :", "rep": masse_g_dosee_attendu},
            {"id": "q31_6", "q": "En deduire la masse de vinaigre dosee en milligramme :", "rep": masse_g_dosee_attendu * 1000.0},
            {"id": "q31_7", "q": "Calculer la concentration massique de vinaigre dosee en g/L :", "rep": cm_g_l_dosee_attendu},
            {"id": "q31_8", "q": "Calculer la concentration massique de vinaigre dosee en mg/L :", "rep": cm_mg_l_dosee_attendu},
        ]

        score31 = 0
        for q in quiz31_inputs:
            rep = st.text_input(q["q"], key=f"inp_{q['id']}").strip().replace(",", ".")
            if rep != "":
                try:
                    if math.isclose(float(rep), float(q["rep"]), rel_tol=5e-2):
                        score31 += 1
                        if not st.session_state.mode_examen_tab31: st.success("Correct")
                    else:
                        if not st.session_state.mode_examen_tab31: st.error("Incorrect")
                except ValueError:
                    if not st.session_state.mode_examen_tab31: st.error("Format numerique invalide")

        if st.button("Valider le Quiz Gauche", key="btn_val_31"):
            st.session_state.examen_verrouille_tab31 = True
            st.success(f"Resultats gauches enregistres. Score running : {score31} / {len(quiz31_inputs)}")

    # --- COLONNE DE DROITE : QUIZ 32 (Déductions sur la bouteille) ---
    with col_workspace_d:
        ...
        quiz32_inputs = [
            {"id": "q32_2", "q": "En deduire le nombre de mole de vinaigre dans la fiole :", "rep": n_fiole_attendu},
            {"id": "q32_3", "q": "En deduire le nombre de mole de vinaigre dans la bouteille :", "rep": n_bouteille_1l_attendu},
            {"id": "q32_4", "q": "Calculer la concentration molaire en vinaigre de la bouteille en mol/L :", "rep": c_bouteille_attendu},
            {"id": "q32_5", "q": "Calculer la masse de vinaigre dans la bouteille en gramme :", "rep": masse_bouteille_g_attendu},
            {"id": "q32_6", "q": "En deduire la masse de vinaigre dans la bouteille en milligramme :", "rep": masse_bouteille_mg_attendu},
            {"id": "q32_8", "q": "Calculer la concentration massique de vinaigre de la bouteille en g/L :", "rep": cm_bouteille_g_l_attendu},
            {"id": "q32_9", "q": "Calculer la concentration massique de vinaigre de la bouteille en mg/L :", "rep": cm_bouteille_mg_l_attendu},
        ]

        score32 = 1 if rep_q32_1 == "10" else 0
        for q in quiz32_inputs:
            rep = st.text_input(q["q"], key=f"inp_{q['id']}").strip().replace(",", ".")
            if rep != "":
                try:
                    if math.isclose(float(rep), float(q["rep"]), rel_tol=5e-2):
                    ...
                    if not st.session_state.mode_examen_tab32: st.error("Format numerique invalide")
        ...
        if rep_conclusion in ["l 'affichage correspond à la valeur trouvée", "l 'affichage correspond  à la valeur trouvée avec une petite différence"]:
            score32 += 1
            if not st.session_state.mode_examen_tab32: st.success("Conclusion coherente")
        elif rep_conclusion != "":
            if not st.session_state.mode_examen_tab32: st.error("Conclusion incorrecte")

        if st.button("Valider le Quiz Droit", key="btn_val_32"):
            st.session_state.examen_verrouille_tab32 = True
            st.success(f"Resultats droits enregistres. Score running : {score32} / 9")
