...
def dessiner_arbre_atelier4(verrouille=False):
    # 1. TRACÉ GÉOMÉTRIQUE DE L'ARBRE (CALQUE INFÉRIEUR DE FOND)
    st.markdown(
        """
        <div style="position: relative; width: 720px; height: 420px; background-color: #ffffff; border: 2px solid #cbd5e1; border-radius: 6px; overflow: hidden; margin: 0 auto; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
            ...
            <!-- BADGES DES ÉVÉNEMENTS GÉOMÉTRIQUES -->
            <div style="position: absolute; top: 92px; left: 200px; font-weight: bold; color: #1e3a8a; border: 2px solid #1e3a8a; background-color: #e0f2fe; padding: 4px 10px; border-radius: 4px; font-size: 11px; z-index: 2;">A</div>
            <div style="position: absolute; top: 302px; left: 200px; font-weight: bold; color: #1e3a8a; border: 2px solid #1e3a8a; background-color: #e0f2fe; padding: 4px 10px; border-radius: 4px; font-size: 11px; z-index: 2;">Ā</div>
            <div style="position: absolute; top: 38px; left: 430px; font-weight: bold; color: #1e3a8a; border: 2px solid #1e3a8a; background-color: #e0f2fe; padding: 4px 10px; border-radius: 4px; font-size: 11px; z-index: 2;">B</div>
            <div style="position: absolute; top: 143px; left: 430px; font-weight: bold; color: #1e3a8a; border: 2px solid #1e3a8a; background-color: #e0f2fe; padding: 4px 10px; border-radius: 4px; font-size: 11px; z-index: 2;">B̄</div>
            <div style="position: absolute; top: 248px; left: 430px; font-weight: bold; color: #1e3a8a; border: 2px solid #1e3a8a; background-color: #e0f2fe; padding: 4px 10px; border-radius: 4px; font-size: 11px; z-index: 2;">B</div>
            <div style="position: absolute; top: 353px; left: 430px; font-weight: bold; color: #1e3a8a; border: 2px solid #1e3a8a; background-color: #e0f2fe; padding: 4px 10px; border-radius: 4px; font-size: 11px; z-index: 2;">B̄</div>

            <div style="position: absolute; top: 43px; left: 485px; font-size: 11px; font-weight: bold; z-index: 2;">P(A &cap; B) =</div>
            <div style="position: absolute; top: 148px; left: 485px; font-size: 11px; font-weight: bold; z-index: 2;">P(A &cap; B̄) =</div>
            <div style="position: absolute; top: 253px; left: 485px; font-size: 11px; font-weight: bold; z-index: 2;">P(Ā &cap; B) =</div>
            <div style="position: absolute; top: 358px; left: 485px; font-size: 11px; font-weight: bold; z-index: 2;">P(Ā &cap; B̄) =</div>
        </div>
        """,
        unsafe_allow_html=True
    )

    # 2. CALQUE SUPÉRIEUR INTERACTIF (BOÎTES NATIVES SANS AUCUNE PERTE)
    st.markdown('<div style="position: relative; z-index: 10; margin-top: -440px; height: 440px; width: 740px; margin-left: auto; margin-right: auto; pointer-events: auto;">', unsafe_allow_html=True)

    scen = st.session_state.get("at4_scenario", {"p_A": 0.5, "p_A_bar": 0.5, "p_B_sachant_A": 0.1, "p_B_bar_sachant_A": 0.9, "p_B_sachant_A_bar": 0.2, "p_B_bar_sachant_A_bar": 0.8, "f1": 0.05, "f2": 0.45, "f3": 0.1, "f4": 0.4})
    afficher_corr = st.session_state.get("at4_afficher_correction", False)

    # Moteur d'injection CSS pour colorer les inputs en Vert ou Rouge
    def injecter_style_at4(cle_w, valeur_attendue, tolerance=0.01):
        if not afficher_corr: return
        saisie = st.session_state.get(cle_w, 0.0)
        is_correct = abs(saisie - valeur_attendue) < tolerance
        c_b = "#10b981" if is_correct else "#ef4444"
        c_f = "#e6f4ea" if is_correct else "#fce8e6"
        c_t = "#137333" if is_correct else "#c5221f"
        st.markdown(f"""<style>div[data-testid="stnumberinput"]:has(input[id="{cle_w}"]) input {{ border: 2px solid {c_b} !important; background-color: {c_f} !important; color: {c_t} !important; font-weight: bold !important; }}</style>""", unsafe_allow_html=True)

    # Placement chirurgical par coordonnées absolues
    # Niveau 1 : Racines
    st.markdown("<div style='position: absolute; top: 125px; left: 80px; width: 75px;'>", unsafe_allow_html=True)
    s_v1 = st.number_input("", min_value=0.0, max_value=1.0, step=0.01, format="%.2f", label_visibility="collapsed", key="v_at4_1_native", disabled=verrouille)
    injecter_style_at4("v_at4_1_native", scen["p_A"])
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 265px; left: 80px; width: 75px;'>", unsafe_allow_html=True)
    s_v2 = st.number_input("", min_value=0.0, max_value=1.0, step=0.01, format="%.2f", label_visibility="collapsed", key="v_at4_2_native", disabled=verrouille)
    injecter_style_at4("v_at4_2_native", scen["p_A_bar"])
    st.markdown("</div>", unsafe_allow_html=True)

    # Niveau 2 : Conditionnelles
    st.markdown("<div style='position: absolute; top: 55px; left: 310px; width: 70px;'>", unsafe_allow_html=True)
    s_v3 = st.number_input("", min_value=0.0, max_value=1.0, step=0.01, format="%.2f", label_visibility="collapsed", key="v_at4_3_native", disabled=verrouille)
    injecter_style_at4("v_at4_3_native", scen["p_B_sachant_A"])
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 145px; left: 310px; width: 70px;'>", unsafe_allow_html=True)
    s_v4 = st.number_input("", min_value=0.0, max_value=1.0, step=0.01, format="%.2f", label_visibility="collapsed", key="v_at4_4_native", disabled=verrouille)
    injecter_style_at4("v_at4_4_native", scen["p_B_bar_sachant_A"])
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 250px; left: 310px; width: 70px;'>", unsafe_allow_html=True)
    s_v5 = st.number_input("", min_value=0.0, max_value=1.0, step=0.01, format="%.2f", label_visibility="collapsed", key="v_at4_5_native", disabled=verrouille)
    injecter_style_at4("v_at4_5_native", scen["p_B_sachant_A_bar"])
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 340px; left: 310px; width: 70px;'>", unsafe_allow_html=True)
    s_v6 = st.number_input("", min_value=0.0, max_value=1.0, step=0.01, format="%.2f", label_visibility="collapsed", key="v_at4_6_native", disabled=verrouille)
    injecter_style_at4("v_at4_6_native", scen["p_B_bar_sachant_A_bar"])
    st.markdown("</div>", unsafe_allow_html=True)

    # Niveau 3 : Intersections
    st.markdown("<div style='position: absolute; top: 38px; left: 565px; width: 85px;'>", unsafe_allow_html=True)
    s_f1 = st.number_input("", min_value=0.0, max_value=1.0, step=0.0001, format="%.4f", label_visibility="collapsed", key="v_at4_f1_native", disabled=verrouille)
    injecter_style_at4("v_at4_f1_native", scen["f1"], 0.001)
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 143px; left: 565px; width: 85px;'>", unsafe_allow_html=True)
    s_f2 = st.number_input("", min_value=0.0, max_value=1.0, step=0.0001, format="%.4f", label_visibility="collapsed", key="v_at4_f2_native", disabled=verrouille)
    injecter_style_at4("v_at4_f2_native", scen["f2"], 0.001)
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 248px; left: 565px; width: 85px;'>", unsafe_allow_html=True)
    s_f3 = st.number_input("", min_value=0.0, max_value=1.0, step=0.0001, format="%.4f", label_visibility="collapsed", key="v_at4_f3_native", disabled=verrouille)
    injecter_style_at4("v_at4_f3_native", scen["f3"], 0.001)
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='position: absolute; top: 353px; left: 565px; width: 85px;'>", unsafe_allow_html=True)
    s_f4 = st.number_input("", min_value=0.0, max_value=1.0, step=0.0001, format="%.4f", label_visibility="collapsed", key="v_at4_f4_native", disabled=verrouille)
    injecter_style_at4("v_at4_f4_native", scen["f4"], 0.001)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    return {
        "p_A": s_v1, "p_A_bar": s_v2, "p_B_sachant_A": s_v3, "p_B_bar_sachant_A": s_v4,
