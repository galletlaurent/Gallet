...
# Donnees metiers issues de votre dictionnaire
if "indicateurs" not in st.session_state:
    st.session_state.indicateurs = {
        ...
        "Helianthine": { "ph_min": 3.1, "ph_max": 4.4,  "couleur_acide": "#E91E63", "nom_acide": "Rouge", "couleur_zone": "#FF5722", "nom_zone": "Orange", "couleur_base": "#FFC107", "nom_base": "Jaune" },
        "Phenolphtaleine": { "ph_min": 8.2, "ph_max": 10.0, "couleur_acide": "#E0F7FA", "nom_acide": "Incolore", "couleur_zone": "#F8BBD0", "nom_zone": "Rose pale",  "couleur_base": "#E91E63", "nom_base": "Rose fuchsia" },
        ...
        "Helianthine / Orange de methyle": {"ph_min": 3.2, "ph_max": 4.4,  "couleur_acide": "#F44336", "nom_acide": "Rouge", "couleur_zone": "#FF9800", "nom_zone": "Orange", "couleur_base": "#FFEB3B", "nom_base": "Jaune" },
        "Vert de Bromocresol": { "ph_min": 3.8, "ph_max": 5.4, "couleur_acide": "#FFEB3B", "nom_acide": "Jaune", "couleur_zone": "#8BC34A", "nom_zone": "Vert", "couleur_base": "#2196F3", "nom_base": "Bleu" },
        "Rouge de Methyle": { "ph_min": 4.2, "ph_max": 6.2, "couleur_acide": "#F44336", "nom_acide": "Rouge","couleur_zone": "#FF5722", "nom_zone": "Orange", "couleur_base": "#FFEB3B", "nom_base": "Jaune" },
        "Bleu de Bromophtenol": { "ph_min": 3.0, "ph_max": 4.6, "couleur_acide": "#FFEB3B", "nom_acide": "Jaune", "couleur_zone": "#00BCD4", "nom_zone": "Vert-Bleu", "couleur_base": "#3F51B5", "nom_base": "Bleu violet" },
        "Phenolphtaleine (Zone large)": { "ph_min": 8.0, "ph_max": 10.0, "couleur_acide": "#E0F7FA", "nom_acide": "Incolore", "couleur_zone": "#F48FB1", "nom_zone": "Rose", "couleur_base": "#C2185B", "nom_base": "Rose soutenu" },
...
# Variables d'etat experimentales et modes examen
...
with header_cols:
    nom_affiche = st.session_state.nom if st.session_state.nom else "Non renseigne"
    prenom_affiche = st.session_state.prenom if st.session_state.prenom else "Non renseigne"
    st.markdown(f"**Nom et Prenom :** {nom_affiche} {prenom_affiche}")
with header_cols:
    classe_affiche = st.session_state.classe if st.session_state.classe else "Non renseignee"
...
tab0, tab1, tab2, tab3 = st.tabs([
    "Identification",
    "Generalites sur le vinaigre",
    "Dosage colorimetrique du vinaigre",
    "Calcul theorique sur le vinaigre et verification de l'inscription sur la bouteille"
])
...
with tab0:
    ...
    with st.container(border=True):
        ...
        prenom_saisi = st.text_input("Prenom :", value=st.session_state.prenom, disabled=st.session_state.verrouille)
...
with tab1:
    ...
    with col_gauche:
        ...
        # Generation de la bouteille 2D via Matplotlib
    ...
    with col_droite:
        ...
        st.divider()
        st.write("**Representation moleculaire de l'acide acetique**")
        ...
        def tracer_liaison(p1, p2, double=False):
            if double:
                ...
                ax_mol.plot([p1[0], p2[0]], [p1[1], p2[1]], color="#333333", linewidth=2, zorder=1)
        ...
        def tracer_atome(p, symbole):
            if symbole == "C":
                couleur, txt_col = "#2b2b2b", "white"
            elif symbole == "O":
                couleur, txt_col = "#e74c3c", "white"
            else:
                couleur, txt_col = "#f1f5f9", "black"
            
            cercle = patches.Circle((p[0], p[1]), 0.22, facecolor=couleur, edgecolor="#1e293b", linewidth=1.5, zorder=2)
            ax_mol.add_patch(cercle)
            ax_mol.text(p[0], p[1], symbole, color=txt_col, weight="bold", fontsize=10, ha="center", va="center", zorder=3)
        ...
        if "ordre_quiz1" not in st.session_state:
            base_quiz1 = [
                ...
                {"id": "q1_5", "q": "Quel est le nombre d'atome d'hydrogene que possede la molecule de vinaigre ?", "type": "entry", "rep": "4"},
                {"id": "q1_6", "q": "Quel est le nombre d'atome d'oxygene que possede la molecule de vinaigre ?", "type": "entry", "rep": "2"},
                {"id": "q1_7", "q": "Quelle est la formule brute du vinaigre ?", "type": "menu", "options": ["C4H2O2", "C2H4O2", "C2H2O4", "C2H2O2"], "rep": "C2H4O2"},
            ]
        ...
        if "examen_verrouille_tab1" not in st.session_state: st.session_state.examen_verrouille_tab1 = False
        if "quiz1_soumis" not in st.session_state: st.session_state.quiz1_soumis = False

        # Barre laterale pour Mode Examen
        with st.sidebar:
        ...
        if st.button("Valider mes reponses", type="primary", disabled=desactiver_validation, key="btn_valider_tab1"):
        ...
        if st.session_state.quiz1_soumis and not st.session_state.mode_examen_tab1:
            st.metric(label=f"Score de {st.session_state.nom.replace(' ', '_')}", value=f"{score1} / {len(st.session_state.ordre_quiz1)}")

        # --- GÉNÉRATION DU RAPPORT HTML ---
        lignes_html_tableau = ""
        for idx, item in enumerate(st.session_state.ordre_quiz1):
            rep_eleve = st.session_state.get(f"select_{item['id']}" if item["type"] == "menu" else f"input_{item['id']}", "").strip()
            
            if rep_eleve == "":
                badge = '<span class="status-fail">INCORRECT</span>'
                rep_eleve_disp = "Aucune reponse"
            elif rep_eleve.lower() == item["rep"].lower():
                badge = '<span class="status-pass">CORRECT</span>'
                rep_eleve_disp = rep_eleve
            else:
                badge = '<span class="status-fail">INCORRECT</span>'
                rep_eleve_disp = rep_eleve
            ...
            <tr>
                ...
                <td>{item['q']}</td>
                <td style="color: #475569;">{rep_eleve_disp}</td>
                <td style="font-weight: 500; color: #1e293b;">{item['rep']}</td>
...
<head>
    <meta charset="UTF-8">
    <title>Resultats et correction - Generalites sur le vinaigre</title>
    <style>
        ...
        .score-box-title {{ font-size: 10px; font-weight: bold; text-transform: uppercase; margin-bottom: 4px; }}
        .score-box-value {{ font-size: 26px; font-weight: bold; color: {'#16a34a' if score1>=4 else '#dc2626'}; }}
        .section-title {{ font-size: 16px; font-weight: bold; color: #1e40af; margin-top: 35px; margin-bottom: 16px; border-bottom: 2px solid #c2e0f0; padding-bottom: 8px; }}
        ...
        .status-fail {{ display: inline-block; padding: 6px 12px; font-size: 11px; font-weight: bold; background-color: #fee2e2; color: #dc2626; border-radius: 4px; }}
    </style>
</head>
<body>
    <div class="header-blue">
        <div class="score-box">
            <div class="score-box-title">NOTE FINALE</div>
            <div class="score-box-value">{score1} / 7</div>
        </div>
        <div class="header-title">Professeur Laurent GALLET</div>
        <div class="meta-info">
            <strong>Eleve :</strong> {nom_eleve_html} {st.session_state.prenom}<br>
            <strong>Evaluation :</strong> QCM - Generalites sur le vinaigre
        </div>
    </div>
    ...
    <table>
        ...
        # Preparation du nom de fichier propre
        ...
        with col_actions:
            ...
            if st.session_state.mode_examen_tab1:
                ...
                if st.button("Recommencer l'exercice", key="reset_tab1_final", use_container_width=True):
                    st.session_state.quiz1_soumis = False
...
with tab2:
    ...
    st.write("Section en cours de preparation.")
...
# --- PIED DE PAGE ---
st.markdown("---")
st.markdown("<div style='text-align: right; color: #6c757d; font-style: italic;'>Cree et developpe par Laurent GALLET</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if "masse_reelle_g" not in st.session_state: st.session_state.masse_reelle_g = random.uniform(80.0, 90.0) / 1000.0
    if "reinit_declenche" not in st.session_state: st.session_state.reinit_declenche = False

    # Recuperation locale des constantes definies dans votre code
    pKa = 4.76
    M_vinaigre = 60.0
    ...
    C_base = st.session_state.c_base if st.session_state.c_base else 0.1
    n_acide_ini = st.session_state.masse_reelle_g / M_vinaigre

    if C_base > 0:
        veq_theorique_mL = (n_acide_ini / C_base) * 1000.0
        # Calcul logarithmique exact du pH a l'equivalence selon votre formule
        concentration_eq = n_acide_ini / ((veq_theorique_mL + v_acide_ml) / 1000.0)
        ph_eq_reel = 0.5 * (pKa + 14.0 + math.log10(concentration_eq))
    else:
        veq_theorique_mL = 0.0
        ph_eq_reel = 7.0

    # Affichage des informations textuelles du flacon
    ...
    # --- FONCTION DE CALCUL DU pH POINT PAR POINT ---
    def extraire_ph_point(v_b_ml):
        v_b = v_b_ml / 1000.0
        v_a_total = v_acide_ml / 1000.0
        n_b = v_b * C_base
        v_tot = v_a_total + v_b
        ...
        if n_b < n_acide_ini:
            ...
            # Equation d'Henderson-Hasselbalch basee sur votre ratio
            return max(1.0, min(13.0, pKa + math.log10(ratio / (1.0 - ratio))))
        else:
            ratio = n_b / n_acide_ini
            if ratio == 1.0:
                return ph_eq_reel
            exces_oh = (C_base * (v_b_ml - veq_theorique_mL)) / (v_acide_ml + v_b_ml)
            pOH = -math.log10(exces_oh) if exces_oh > 0 else 7.0
            return max(1.0, min(13.9, 14.0 - pOH))

    # Generation matricielle des points du graphique complet en arriere-plan
    volumes_simules = np.arange(0, v_max_ml + 0.1, 0.1)
    phs_simules = [extraire_ph_point(v) for v in volumes_simules]

    # --- AJOUT PAS-A-PAS INTERACTIF ---
    ...
    with col_ctrl1:
        if st.button("Verser une goutte (+ pas selectionne)", use_container_width=True):
            if st.session_state.v_verse + st.session_state.pas_ml <= v_max_ml:
                st.session_state.v_verse = round(st.session_state.v_verse + st.session_state.pas_ml, 2)
            else:
                st.session_state.v_verse = v_max_ml
            st.rerun()
    with col_ctrl2:
        if st.button("Vider la burette (Reinitialiser la simulation)", use_container_width=True):
            st.session_state.v_verse = 0.0
            st.rerun()

    # Recuperation des donnees du point actuel de l'eleve
    idx_actuel = min(int(st.session_state.v_verse * 10), len(volumes_simules) - 1)
    ph_actuel = ph_simules[idx_actuel]

    # --- MISE EN PAGE INTERACTIVE : SCHÉMA DU MONTAGE & GRAPHIC ---
    ...
    with col_visuel:
        ...
        # 2. Burette graduee et son niveau de liquide
        ...
        ax_mo.add_patch(patches.Rectangle((3.1, 1.1), 1.2, 0.15, facecolor="white", edgecolor="#7f8c8d", angle=angle_barreau))
        
        # 6. Sonde pH-metrique plongeante
        ...
        # 7. Boitier afficheur du pH-metre digital
        ax_mo.add_patch(patches.Rectangle((5.5, 6.5), 2.2, 1.5, facecolor="#2c3e50", edgecolor="#1a252f", linewidth=2))
        ax_mo.text(6.6, 7.2, f"pH = {ph_actuel:.2f}", color="white", fontfamily="monospace", weight="bold", fontsize=11, ha="center")
    ...
    with col_graph:
        ...
        ax_cu.plot(volumes_simules, ph_simules, color="#cbd5e1", linestyle="--", label="Courbe entiere")
    ...
    # --- TABLEAU DE SUIVI DES MESURES TRANSPOSE ---
    ...
    for idx in indices_mesures:
        ...
        ph_pt = ph_simules[idx]
        
        if ph_pt < ind_data["ph_min"]: obs = ind_data["nom_acide"]
        elif ph_pt > ind_data["ph_max"]: obs = ind_data["nom_base"]
        else: obs = ind_data["nom_zone"]
    ...
    st.subheader("Quiz 2 : Formulaire d'evaluation du dosage")
     ...
     # --- INITIALISATION DE L'ORDRE DU QUIZ 2 ---
    if "ordre_quiz2" not in st.session_state:
        base_quiz2 = [
            {"id": "q2_1", "q": "Quel indicateur colore est le mieux adapte au dosage ?", "type": "menu", "options": ["Helianthine", "Bleu de Thymol", "Bleu de Bromothymol (BBT)", "Jaune d'Alizarin R", "Phenolphtaleine"], "rep": "Phenolphtaleine"},
            {"id": "q2_2", "q": "Quel est le role d'un indicateur colore ?", "type": "menu", "options": ["mettre une couleur dans la solution de depart", "reperer l'equivalence", "connaitre la valeur du pH"], "rep": "reperer l'equivalence"},
    ...
    for item in st.session_state.ordre_quiz2:
        if item["type"] == "menu":
            ...
            est_correct = (str(reponses_eleve_q2[item["id"]]).strip().lower() == bonne_rep)
        else:
            reponses_eleve_q2[item["id"]] = st.text_input(item["q"], key=f"f_inp_q2_{item['id']}").strip().replace(",", ".")
            ...
            if "volume" in q_lower and "equivalent" in q_lower:
                ...
                est_correct = (str(val_saisie).lower() == target)

        if reponses_eleve_q2[item["id"]] != "":
            if est_correct:
                score2 += 1
                if not st.session_state.mode_examen_tab2:
                    st.success("Correct")
            else:
                if not st.session_state.mode_examen_tab2:
                    st.error("Incorrect")
    ...
    if st.button("Valider et enregistrer le Quiz 2", key="btn_soumission_q2"):
        if not st.session_state.nom.strip() or not st.session_state.prenom.strip():
            ...
            st.session_state.v_eq = veq_theorique_mL
    ...
    with st.sidebar:
        ...
        if mode_examen_actif2 and not st.session_state.mode_examen_tab2:
            ...
            st.session_state.examen_verrouille_tab2 = True
            st.session_state.quiz2_soumis = False
            random.shuffle(st.session_state.ordre_quiz2)
            for item in st.session_state.ordre_quiz2:
                if f"sel_q2_{item['id']}" in st.session_state: st.session_state[f"sel_q2_{item['id']}"] = ""
                if f"f_inp_q2_{item['id']}" in st.session_state: st.session_state[f"f_inp_q2_{item['id']}"] = ""
            st.rerun()
    ...
    # --- LOGIQUE D'EXPORTATION DU RAPPORT HTML ---
    lignes_html_tableau2 = ""
    score2 = 0
    ...
    if total_questions > 0:
        for idx, item in enumerate(st.session_state.ordre_quiz2):
            ...
            if item["type"] == "menu":
                ...
                reponse_eleve = st.session_state.get(f"f_inp_q2_{item['id']}", "").strip()
            ...
            if "volume" in q_lower and "equivalent" in q_lower:
                reponse_correcte = f"{veq_theorique_mL:.2f}"
            elif "ph" in q_lower and "equivalent" in q_lower:
                reponse_correcte = f"{ph_eq_reel:.2f}"
            elif "concentration" in q_lower:
                reponse_correcte = f"{C_base:.2f}"
            else:
                reponse_correcte = str(item["rep"])
...
<body>
    ...
    <table>
        ...
        # Affichage du bouton de telechargement web
        st.markdown("---")
        if not st.session_state.nom.strip() or not st.session_state.prenom.strip():
...
with tab3:
    ...
    # --- 1. ZONE DE RAPPEL DU PROTOCOLE ET DES RESULTATS ---
    with st.container(border=True):
        ...
        # Recuperation securisee des valeurs experimentales de l'onglet 2
        v_eq_ml = st.session_state.get("v_eq", 0.0)
        c_titrant = st.session_state.get("c_titrant", 0.1)
        ph_eq = st.session_state.get("ph_eq", 7.0)
        v_titre_ml = 10.0
        masse_molaire = 60.0

        col_rap_g, col_rap_d = st.columns(2)
        with col_rap_g:
            st.markdown("**Donnees de votre dosage**")
        ...
        with col_rap_d:
            st.markdown("**Constantes du systeme**")
            st.markdown(f" Volume de solution titre $V_a$ = {v_titre_ml:.1f} mL")
            st.markdown(f" Masse molaire de l'acide acetique = {masse_molaire:.1f} g/mol")

    st.divider()

    # --- 2. CONFIGURATION DU MODE EXAMEN INDEPENDANT (Sidebar) ---
    ...
    val_n_soude = c_titrant * v_l_attendu
    val_n_vinaigre = val_n_soude
    val_c_vinaigre = val_n_vinaigre / (v_titre_ml / 1000.0) if v_titre_ml > 0 else 0.0
    val_masse_g = val_n_vinaigre * masse_molaire
    val_masse_mg = val_masse_g * 1000.0
    val_cm_g = val_c_vinaigre * masse_molaire
    val_cm_mg = val_cm_g * 1000.0

    # Recalculs analytiques pour la bouteille commerciale (dilution 10 fois)
    rapport_dilution = 10.0
    val_n_fiole = val_n_vinaigre * (100.0 / 10.0)
    val_n_bouteille = val_n_fiole
    val_c_bouteille = val_c_vinaigre * rapport_dilution
    val_masse_bouteille_g = val_n_bouteille * masse_molaire
    val_masse_bouteille_mg = val_masse_bouteille_g * 1000.0
    val_cm_bouteille_g = val_c_bouteille * masse_molaire
    ...
    # --------------------------------------------------------
    # COLONNE DE GAUCHE : QUIZ 31 (Analyse de la solution dosee)
    # --------------------------------------------------------
    with col_workspace_g:
        st.subheader("Quiz Droite : Analyse de la solution dosee")
        
        if "ordre_quiz31" not in st.session_state:
            base_quiz31 = [
                {"id": "q31_1", "q": "Convertir le volume equivalent en litre :", "type": "entry"},
                {"id": "q31_2", "q": "Calculer le nombre de mole de soude versee :", "type": "entry"},
                {"id": "q31_3", "q": "En deduire le nombre de mole de vinaigre dosee :", "type": "entry"},
                {"id": "q31_4", "q": "Calculer la concentration molaire en vinaigre dosee en mol/L :", "type": "entry"},
                {"id": "q31_5", "q": "Calculer la masse de vinaigre dosee en gramme :", "type": "entry"},
                {"id": "q31_6", "q": "En deduire la masse de vinaigre dosee en milligramme :", "type": "entry"},
                {"id": "q31_7", "q": "Calculer la concentration massique de vinaigre dosee en g/L :", "type": "entry"},
                {"id": "q31_8", "q": "Calculer la concentration massique de vinaigre dosee en mg/L :", "type": "entry"},
            ]
            st.session_state.ordre_quiz31 = base_quiz31

        if "quiz31_soumis" not in st.session_state: st.session_state.quiz31_soumis = False
        score31 = 0
        reponses_eleve_q31 = {}
        ...
        for item in st.session_state.ordre_quiz31:
            ...
            if "convertir le volume" in q_lower: bonne_rep = v_l_attendu
            elif "soude versee" in q_lower: bonne_rep = val_n_soude
            elif "mole de vinaigre" in q_lower: bonne_rep = val_n_vinaigre
            ...
            correction_dynamique_q31[item["id"]] = bonne_rep
        ...
        if st.button("Valider le Quiz Gauche", key="btn_val_31"):
            if not st.session_state.nom.strip() or not st.session_state.prenom.strip():
                st.error("Action impossible ! Veuillez inscrire votre NOM et votre PRENOM dans l'onglet Identification.")
            else:
                st.session_state.examen_verrouille_tab31 = True
                st.session_state.quiz31_soumis = True
                st.success(f"Resultats gauches enregistres. Score running : {score31} / {len(st.session_state.ordre_quiz31)}")
                st.rerun()

        if st.session_state.quiz31_soumis and not st.session_state.mode_examen_tab31:
            if st.button("Recommencer le Quiz Gauche (Entrainement)", key="reset_q31_tree"):
                st.session_state.quiz31_soumis = False
                st.session_state.examen_verrouille_tab31 = False
                for item in st.session_state.ordre_quiz31:
                    if f"inp_q31_{item['id']}" in st.session_state: st.session_state[f"inp_q31_{item['id']}"] = ""
                st.rerun()

    # --------------------------------------------------------
    # COLONNE DE DROITE : QUIZ 32 (Deductions sur la bouteille)
    # --------------------------------------------------------
    with col_workspace_d:
        st.subheader("Quiz Droit : Analyse de la bouteille commerciale")
        
        rep_q32_1 = st.text_input("Donner le rapport de dilution ?", key="inp_q32_1").strip()
        score32_init = 1 if rep_q32_1 == "10" else 0
        if rep_q32_1 != "":
            if rep_q32_1 == "10":
        ...
        if "ordre_quiz32" not in st.session_state:
            base_quiz32 = [
                {"id": "q32_2", "q": "En deduire le nombre de mole de vinaigre dans la fiole :", "type": "entry"},
                {"id": "q32_3", "q": "En deduire le nombre de mole de vinaigre dans la bouteille :", "type": "entry"},
                ...
                {"id": "q32_6", "q": "En deduire la masse de vinaigre dans la bouteille en milligramme :", "type": "entry"},
                {"id": "q32_7", "q": "Calculer la concentration massique de vinaigre de la bouteille en g/L :", "type": "entry"},
                {"id": "q32_8", "q": "Calculer la concentration massique de vinaigre de la bouteille en mg/L :", "type": "entry"},
            ]
            st.session_state.ordre_quiz32 = base_quiz32

        if "quiz32_soumis" not in st.session_state: st.session_state.quiz32_soumis = False
        score32_body = 0
        reponses_eleve_q32 = {}
        correction_dynamique_q32 = {}

        for item in st.session_state.ordre_quiz32:
            q_low = item["q"].lower()
            
            if "dans la fiole" in q_low: bonne_rep = val_n_fiole
            elif "dans la bouteille" in q_low and "mole" in q_low: bonne_rep = val_n_bouteille
            elif "concentration molaire" in q_low: bonne_rep = val_c_bouteille

            elif "en gramme" in q_low: bonne_rep = val_masse_bouteille_g
            elif "en milligramme" in q_low: bonne_rep = val_masse_bouteille_mg
            elif "g/l" in q_low: bonne_rep = val_cm_bouteille_g
            elif "mg/l" in q_low: bonne_rep = val_cm_bouteille_mg
            else: bonne_rep = 0.0

            correction_dynamique_q32[item["id"]] = bonne_rep
            reponses_eleve_q32[item["id"]] = st.text_input(item["q"], key=f"f_sel_q32_{item['id']}").strip().replace(",", ".")
        ...
        # --- LOGIQUE D'EXPORTATION DU RAPPORT HTML GAUCHE ---
        ...
        if total_questions31 > 0:
            for idx, item in enumerate(st.session_state.ordre_quiz31):
                ...
                bonne_rep = correction_dynamique_q31.get(item["id"], 0.0)

                reponse_correcte = f"{bonne_rep:.4f}" if bonne_rep != 0 else "0"
...
<body>
    ...
    # --------------------------------------------------------
    with col_workspace_d:
        # --- LOGIQUE D'EXPORTATION DU RAPPORT HTML DROIT ---
        ...
        if total_questions32 > 0:
            for idx, item in enumerate(st.session_state.ordre_quiz32):
                intitule_q = item.get("q", "").strip()
                reponse_eleve = st.session_state.get(f"f_sel_q32_{item['id']}", "").strip()
                bonne_rep = correction_dynamique_q32.get(item["id"], 0.0)
                ...
                if est_num:
                    ...
                    reponse_correcte = "L'affichage correspond a la valeur trouvee"
                ...
                if reponse_eleve == "":
                    ...
                    if est_num:
                        ...
                        est_correct = reponse_eleve in ["l'affichage correspond a la valeur trouvee", "l'affichage correspond a la valeur trouvee avec une petite difference"]
